import { prepareDebitDataMulticall, parseDebitDataResult, type DebitData } from "@1delta/calldata-sdk"
import { getBestRpcsForChain } from "@1delta/lib-utils"
import { multicallRetryUniversal } from "@1delta/providers"
import { useQuery } from "@tanstack/react-query"

function createDebitQueryFn(params: { chainId: string; tokenAddresses: string[]; account: string; spenders: string[]; allTokenLists: any }) {
    ...
    return async (): Promise<Record<string, DebitData>> => {
        ...
        const overrides = rpcFromRpcSelector && rpcFromRpcSelector.length > 0 ? { [chainId]: rpcFromRpcSelector } : undefined
