...
export function rawMarketToPoolDataItem(raw: RawMarket, entry?: LenderEntryRaw): PoolDataItem {
  ...
  return {
    ...
    // Coerce the string-serialized rate card into clean numbers. Passed
    // through as served: a card is a BORROW-side fact and the API guarantees a
    // row with `flags.borrowingEnabled === false` never carries one (enforced
    // on the origin's `market_terms` table). Whether a row can be BORROWED is
    // read off the flags via `marketSides.ts`, never off the card.
    terms: raw.terms
      ? raw.terms.map((t) => ({
