...
function buildPath(
  ...
  return points
    .map(
      (p, i) =>
        `${i === 0 ? 'M' : 'L'}${scaleX(p.utilization ?? 0, w).toFixed(1)},${scaleY(p[key] ?? 0, maxRate, h).toFixed(1)}`
    )
    .join(' ')
}
...
export const IrmCurveChart: React.FC<IrmCurveChartProps> = ({ points, currentUtilization }) => {
  ...
  const resolveU = useCallback(
    (clientX: number): number => {
      const rect = svgRef.current!.getBoundingClientRect()
      return Math.max(
        0,
        Math.min(1, (clientX - rect.left - PAD.left) / (width - PAD.left - PAD.right))
      )
    },
    [width]
  )
      ...
      >
        ...
        {yTicks.map((v) => (
          <line
            ...
            x1={PAD.left}
            x2={width - PAD.right}
            y1={scaleY(v, maxRate, height)}
            y2={scaleY(v, maxRate, height)}
            stroke="currentColor"
            strokeOpacity={0.07}
            strokeWidth={1}
          />
        ...
        {currentUtilization !== undefined && (
          <line
            x1={scaleX(currentUtilization, width)}
            x2={scaleX(currentUtilization, width)}
            y1={PAD.top}
            y2={height - PAD.bottom}
            stroke="currentColor"
            strokeOpacity={0.35}
        ...
        {hovered && (
          <>
            <line
              x1={scaleX(hovered.utilization, width)}
              x2={scaleX(hovered.utilization, width)}
              y1={PAD.top}
              y2={height - PAD.bottom}
              stroke="currentColor"
              strokeOpacity={0.2}
            ...
            />
            <circle
              cx={scaleX(hovered.utilization, width)}
              cy={scaleY(hovered.depositRate, maxRate, height)}
              r={4}
              style={{ fill: DEPOSIT_COLOR }}
            />
            <circle
              cx={scaleX(hovered.utilization, width)}
              cy={scaleY(hovered.borrowRate, maxRate, height)}
              r={4}
              style={{ fill: BORROW_COLOR }}
            />
          </>
        )}
      </svg>
        ...
        >
          <p className="text-base-content/50 mb-1.5 tabular-nums text-[10px]">
            ...
            <span className="text-base-content font-semibold">
              {(hovered.utilization * 100).toFixed(0)}%
            </span>
          </p>
          <div className="flex items-center gap-2 tabular-nums">
            ...
            <span
              className="font-semibold ml-auto pl-3 text-[10px]"
