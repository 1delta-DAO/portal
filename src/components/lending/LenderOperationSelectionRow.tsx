...
import {
  useLenderSelection,
  type LenderOperationSelection,
  type LenderOperationKind,
} from '../../contexts/LenderSelectionContext'
...
export const LenderOperationSelectionRow: React.FC<LenderOperationSelectionRowProps> = ({
  selection,
  pools,
  simulated,
  price = 0,
}) => {
  const { setSelectionPool, setSelectionAmount, setSelectionOperation, removeSelection } =
    useLenderSelection()
  ...
  const borrowCapacityAfter =
    simulated != null
      ? simulated.balanceAfter.borrowDiscountedCollateral - simulated.balanceAfter.adjustedDebt
      : undefined
      ...
      <div className="flex flex-col gap-2 rounded-box border border-base-300 p-3 bg-base-100">
        ...
        <div className="grid gap-2 items-start grid-cols-1 md:grid-cols-[25%_10%_65%]">
          ...
          <OperationSelect
            ...
            onChange={(op) => handleOperationChange(op)}
          />
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {userEntries.map(([subId, pos]) => {
                      ...
                      return (
                        ...
                        >
                          {label && (
                            <span className="font-semibold uppercase truncate w-full">{label}</span>
                          )}
                          {(dep > 0 || debtTotal > 0) && (
                            <span className="font-semibold uppercase truncate w-full">
                              Position
                            </span>
                          )}
                          {dep > 0 && (
