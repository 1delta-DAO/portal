...
export const LendingPoolsTable: React.FC<LendingPoolsTableProps> = ({ chainId }) => {
    ...
    const lenders = useMemo(() => Array.from(new Set(poolsForChain.map((p) => p.lender))).sort(), [poolsForChain])
    ...
    const filteredAndSortedPools = useMemo(() => {
        ...
        if (assetFilter.trim()) {
            ...
            result = result.filter((p) => {
                ...
                return symbol.toLowerCase().includes(q) || name.toLowerCase().includes(q) || address.toLowerCase().includes(q)
            })
        }
        ...
        return result
    }, [poolsForChain, search, selectedLender, sortKey, sortDir, maxUtilPct, minDepositsUsd, minAprPct, assetFilter])
    ...
    useEffect(() => {
        setPage(1)
    }, [search, selectedLender, sortKey, sortDir, pageSize, maxUtilPct, minDepositsUsd, minAprPct, assetFilter, chainId])
        ...
        <div className="w-full max-w-6xl mx-auto p-4 space-y-4">
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                <div>
                    ...
                    <p className="text-sm text-base-content/70">Flattened view of pools enriched with APR, utilization and exposure.</p>
                ...
                <div className="flex flex-wrap gap-2 md:justify-end">
                    ...
                    <select className="select select-bordered select-sm" value={selectedLender} onChange={(e) => setSelectedLender(e.target.value)}>
                    ...
                    {/* page size selector */}
                    <select className="select select-bordered select-sm" value={pageSize} onChange={(e) => setPageSize(parseInt(e.target.value, 10))}>
            ...
            <div className="rounded-box border border-base-300 overflow-visible">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th className="cursor-pointer" onClick={() => toggleSort("apr")}>
                                    APR
                                    {sortKey === "apr" && <span className="ml-1 text-xs">{sortDir === "asc" ? "↑" : "↓"}</span>}
                                </th>
                                <th className="cursor-pointer" onClick={() => toggleSort("utilitzation")}>
                                    Utilization
                                    {sortKey === "utilitzation" && <span className="ml-1 text-xs">{sortDir === "asc" ? "↑" : "↓"}</span>}
                                </th>
                            ...
                            {paginatedPools.map((p) => {
                                ...
                                return (
                                    <tr key={`${p.chainId}-${p.lender}-${p.poolId}`} className="h-[75px]">
                                        ...
                                        <td>
                                            <div className="flex items-center gap-2">
                                                <progress className="progress progress-primary w-24" value={utilPct} max={100} />
                ...
                <div className="flex flex-col gap-2 items-center justify-between md:flex-row px-4 py-2">
                    ...
                    <div className="join">
                        <button className="btn btn-xs join-item" disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)}>
                            « Prev
                        </button>
                        <button className="btn btn-xs join-item" disabled>
                            Page {currentPage} / {totalPages}
                        </button>
                        <button className="btn btn-xs join-item" disabled={currentPage === totalPages} onClick={() => goToPage(currentPage + 1)}>
                            Next »
                        </button>
                    </div>
                </div>
